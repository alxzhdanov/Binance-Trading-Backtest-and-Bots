...
def symbols():
    ...
    sym = [ 'XRP','BCH','ADA','BAT','MATIC','VET','GRT','DOGE','COMP','LINK','SNX','YFI','CAKE','DOT','FIO','MKR','BNB','ZEC','EGLD','ZIL','EOS','LTC','XLM','XTZ','ETC']
...
def orders():
    ...
    max_index = Abs.idxmax()
    
    
    
    
    
    sym = symbol[max_index]
    cross = sym + 'USDT'
    ...
    for i in range(len(acct['userAssets'])):
        if df['Direction'][max_index] == 1 and acct['userAssets'][i]['asset'] == sym:
            desired = (float(amount-10))/float(cp)
    ...
    for i in range(len(acct['userAssets'])):
        ...
        #client.create_margin_order(symbol=cross,  side = SIDE_SELL, type = 'MARKET',  quantity=(quantity_s))
            client.create_margin_loan(asset=sym, amount=quantity_bs)
