# -*- coding: utf-8 -*-
"""
Created on Wed May  5 11:34:06 2021
...
def gnb_pred(symbol, datefrom):
    ...
    for i in range(look_back, len(df)):
        ...
        predictions.append(model.fit(df[cols][i-look_back:i],df['Returns'][i-look_back:i]).predict(df[cols][i:i+1]))
       
    df = df[look_back:len(df)]
    
    predictions = np.array(predictions)
    
    df['pos_clus'] = np.where(predictions > -0.25, 1, -1)
...
print(gnb_pred('SNXBUSD', '26 Feb 2020'))